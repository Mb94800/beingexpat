...
class LoginViewController : UIViewController, UIActionSheetDelegate {
    ...
    func authAccount(_ account: ACAccount) {
        authHelper.authenticateAccount(account, withCallback: { (error, authData) -> Void in
            if error != nil {
                ...
                NSLog("%@", authData)
                // segue to chat
                self.performSegue(withIdentifier: "TWITTER_LOGIN", sender: authData)
            }
        })
    }
    
    func selectTwitterAccount(_ accounts: [ACAccount]) {
        ...
        for account in accounts {
            selectUserActionSheet.addButton(withTitle: account.username)
        }
        
        selectUserActionSheet.cancelButtonIndex = selectUserActionSheet.addButton(withTitle: "Cancel")
        selectUserActionSheet.show(in: self.view);
    }
    
    func handleMultipleTwitterAccounts(_ accounts: [ACAccount]) {
    ...
    func actionSheet(_ actionSheet: UIActionSheet, clickedButtonAt buttonIndex: Int) {
        let currentTwitterHandle = actionSheet.buttonTitle(at: buttonIndex)
        for acc in accounts {
